      ...
      _pq_table(std::make_unique<FixedChunkPQTable>())
{
    ...
    {
        _pq_table->load_pq_centroid_bin(files, codeboodk_path.c_str(), num_pq_chunks);
    }
#else
    if (!codebook_path.empty())
    {
...
{
    if (_pq_table == nullptr)
    {
        throw diskann::ANNException("PQ Table is not loaded, cannot set vector for PQDataStore.", -1);
    }

    uint64_t full_dimension = _pq_table->get_num_dims();
    ...
    generate_pq_data_from_pivots_simplified(vector_float.data(), 1, _pq_table->get_tables(), 256 * full_dimension,
                                            full_dimension,
